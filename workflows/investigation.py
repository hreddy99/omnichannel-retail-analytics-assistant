...
from data.generator import build_duckdb, get_meta, get_tie_meta
...
def _setup(question, seed, use_index, inject_failure, top_k, beam_width, depth,
           inject_tie, inject_stale):
    con = build_duckdb(seed)
    audit = AuditLog()
    # The tie scenario investigates the seeded co-equal incident day (real data, real
    # scoring) instead of yesterday, no score injection.
    meta = get_tie_meta(seed) if inject_tie else get_meta(seed)
...
def run_investigation(question: str, seed: int = 42, use_index: bool = True,
    ...
    inject_tie forces an equal-strength tie; inject_stale simulates a stale source (demo only).
    """
    con, audit, state = _setup(question, seed, use_index, inject_failure, top_k,
                               beam_width, depth, inject_tie, inject_stale)
...
def run_investigation_stream(question: str, seed: int = 42, use_index: bool = True,
    ...
    state = {"question": question, "con": con, "g": g, "meta": meta,
             ...
             "inject_tie": inject_tie, "inject_stale": inject_stale,
             "top_k": top_k, "beam_width": beam_width, "depth": depth}
    result: dict = {}
    seen = 0
