...
def n_sync_gate(state: WState) -> dict:
    ...
    in_sync = bool(ok and sync["in_sync"])
    # Real data-recency check on a governed source (the inventory feed). The stale
    # scenario represents that feed arriving a day late; the gate detects the lag from
    # the data itself, the source's latest load vs the T-1 freshness SLA, and routes
    # the run to human review rather than answering on stale context (YAML/governed
    # freshness wins). For a normal run the feed is current, so the gate passes.
    con = state.get("con")
    target_day = (state.get("meta") or {}).get("target_day")
    lag_days = 1 if state.get("inject_stale") else 0
    if con is not None and target_day:
        try:
            row = con.execute("SELECT max(date) FROM fact_inventory_daily").fetchone()
        except Exception as e:
            row = None
            # Don't silently mask a broken feed, record that the recency check couldn't run.
            _step(state, "sync gate · freshness check skipped",
                  f"Could not read fact_inventory_daily recency ({type(e).__name__}); "
                  "freshness not verified.", ok=False)
        if row and row[0] is not None:
            latest = dt.date.fromisoformat(str(row[0])) - dt.timedelta(days=lag_days)
            tgt = dt.date.fromisoformat(str(target_day))
            if latest < tgt:
                in_sync = False
                behind = (tgt - latest).days
                msg = (f"Stale source: fact_inventory_daily latest load {latest.isoformat()} is "
                       f"{behind} day(s) behind the T-1 freshness SLA (expected {tgt.isoformat()}); "
    ...
    a.event(workflow_node="sync_gate", decision_type="catalog_sync_checked",
            ...
            output_summary=f"{msg} retrieval in_sync={in_sync}",
            status="success" if in_sync else "blocked",
            user_visible_note="Catalog/graph/vector versions verified in sync." if in_sync
            else "Stale source flagged for refresh; routed to human review.")
    _step(state, "sync gate", f"{msg} Vector index in_sync={in_sync} "
          f"(embedder: {sync.get('embedder')}).", in_sync)
    return {"sync": {"in_sync": in_sync, "embedder": sync.get("embedder")}}
...
def n_human_review(state: WState) -> dict:
    ...
    if degraded:
        names = ", ".join(d.get("agent", "an analyst") for d in degraded)
        were = "were" if len(degraded) > 1 else "was"
        reasons.append(f"the {names} failed/timed out and {were} excluded, the drivers were "
                       "ranked on partial evidence and need confirmation before acting")
        risk = max_risk(risk, "medium")
    if tie:
        ...
        risk = max_risk(risk, "high")
    if intent == "briefing":
        reasons.append("executive-facing briefing spans multiple teams, high business impact")
        risk = max_risk(risk, "medium")

    # A routine, well-supported investigation (clear drivers, fresh data, no tie/failure)
    # does NOT force review, the assistant still only recommends (no system writes), and the
    # action plan notes findings are owner-reviewed. Human review fires only on the Checkpoint-6
    # conditions above (write/PII, inconclusive, stale/conflict, degraded, tie, high-impact briefing).
    if not reasons:
        a.event(workflow_node="human_review_gate", decision_type="human_review_not_required",
                tool_name="Guardrails", output_summary="no safety condition met",
                user_visible_note="Routine investigation, owner-routed recommendations; no review gate triggered.")
        _step(state, "human review", "No safety condition met, recommendations routed to owners "
              "for action (read-only; no system write). No review gate triggered.")
        return {"review": None}
...
def n_synthesize(state: WState) -> dict:
    ...
    if intent == "driver" and fb is not None:
        # A single-domain question still runs the full analyst team (multi-owner), so present
        # the same owner-routed, prioritized executive summary as the cross-domain
        # investigation, the focused driver leads the bullets, then the full action plan.
        es_title = "Executive summary"
        bullets = ([f"Focus, {fb.label} → **{fb.owner}**: {fb.finding} (confidence: {fb.confidence}).",
                    f"Context: {ctx}."]
