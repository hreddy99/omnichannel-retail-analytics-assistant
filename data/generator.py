...
# Edge-case "tie" incident day: a real, pre-ramp day (15 days before yesterday, so its
# own prior-7 baseline is clean) on which the Electronics stockout and the West
# fulfillment delay are calibrated to near-equal relative magnitude. Investigated on its
# own, the two drivers score equally and land within the tie epsilon, so the deterministic
# tie-break genuinely cannot separate them (no score injection — the tie emerges from data).
TIE_LAG_DAYS = 15
TIE_STOCKOUT = 0.165       # Electronics stockout on the tie day (calibrated to match delay signal)
TIE_DELAY_DAYS = 2.55      # West delivery delay (actual-promise) on the tie day (calibrated)
TIE_CONV_DROP_FRAC = 0.45  # fraction of affected (C01/west) conversions dropped on the tie day
...
def generate(seed: int = SEED) -> dict[str, pd.DataFrame]:
    ...
    # Isolated stream used ONLY to post-stamp the tie incident day, so it never advances
    # the main rng and the rest of the seeded dataset stays byte-for-byte identical.
    rng_tie = np.random.default_rng(seed + 777)
    np.random.seed(seed)
    fake = Faker()
    Faker.seed(seed)
    ...
    dates = _dates()
    target_day = dates[-1]
    tie_day = target_day - dt.timedelta(days=TIE_LAG_DAYS)
    ...
    for d in dates:
        is_t = d == target_day
        is_tie = d == tie_day            # edge-case: two co-equal drivers (see TIE_* knobs)
        ...
        # ---- tie incident day: drop a deterministic share of the affected (Electronics /
        # west) conversions so the day shows a real conversion drop. Done AFTER the orders
        # are built and with an isolated rng, so the main stream — and therefore yesterday's
        # seeded drop — is unchanged.
        if is_tie:
            sf = sess_frames[-1]
            cv = sf["converted"].to_numpy().copy()
            affected = ((sf["category_id"].to_numpy() == SCEN["inventory_category"])
                        | (sf["region"].to_numpy() == SCEN["fulfillment_region"])) & cv
            cv[affected & (rng_tie.random(len(cv)) < TIE_CONV_DROP_FRAC)] = False
            sf["converted"] = cv
        ...
        for cid in CAT_IDS:
            ...
            if is_tie and cid == SCEN["inventory_category"]:
                views = int(views * 1.3); stockout = TIE_STOCKOUT
        ...
        for reg in REGIONS:
            for ft in FULFILLMENT_TYPES:
                ...
                if is_tie and reg == SCEN["fulfillment_region"]:
                    # RNG-neutral: overwrite delay/options/cancels without drawing new
                    # randoms, so the seeded stream for every other day is unchanged.
                    actual = promise + TIE_DELAY_DAYS
                    options = 3; cancels = cancels + 16
    ...
    frames["_meta"] = pd.DataFrame([{
        # baseline_start..baseline_end is the prior-7-day comparison window (the analysis
        # baseline); n_baseline_days describes that window, not the count of days generated.
        "target_day": target_day, "baseline_start": dates[-8], "baseline_end": dates[-2],
        "n_baseline_days": 7,
        "tie_target_day": tie_day, "tie_baseline_start": tie_day - dt.timedelta(days=7),
        "tie_baseline_end": tie_day - dt.timedelta(days=1)}])
    return frames
...
def get_tie_meta(seed: int = SEED) -> dict:
    """Meta windowed onto the seeded co-equal 'tie' incident day (15 days before
    yesterday), so the investigation runs the real pipeline on that day's data and a
    genuine unresolved tie emerges between inventory and fulfillment."""
    m = generate(seed)["_meta"].iloc[0]
    return {"target_day": str(m["tie_target_day"]), "baseline_start": str(m["tie_baseline_start"]),
            "baseline_end": str(m["tie_baseline_end"]), "n_baseline_days": 7}
