...
## Instructions
1. In `workflows/insights.py`, add an `Insight(id, question, domain, owner, sql, summarize)` to
...
4. The classifier and the `n_analytics` workflow node route it automatically; it appears
   in the Run Analysis dropdown under "Direct analytics questions".
5. Verify: `python -c "from workflows.insights import run; ..."` and the AppTest smoke checks.

Reference implementations: the 11 existing insights in `workflows/insights.py`.
