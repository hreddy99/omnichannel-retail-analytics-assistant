...
real LangGraph pipeline and exposes seven review tabs (business answer, multi-agent
team, evidence, trust details, ToT trace, technical audit, and action log).
...
# Global readability: the base font size is set via theme.baseFontSize in
# .streamlit/config.toml (the supported lever in Streamlit ≥1.5x). Here we only add a little
# extra line-height to body copy for easier reading.
st.markdown(
    """
    <style>
...
_CCY = ("sales", "revenue", "gross", "net", "spend", "returns", "amount",
        "lost_sales", "gmv", "cac")
_CCY_CENTS = ("price", "order_value", "aov", "cost_per", "per_unit", "unit_cost")
...
def page_overview():
    ...
    st.subheader("System goal & scope")
    st.write(P.SYSTEM_GOAL_SCOPE)
...
def _arch_main():
    st.write("LangGraph is the central controller. Tools and knowledge layers do not act "
             ...
             "The controller flow and reasoning loop are shown in the Reference diagrams above.")
...
def _biz_chart(t, key):
    """Render the question's chart with vertical breathing room so it isn't crowded by the
    surrounding text (used across all business-answer intents for consistent spacing)."""
...
def _tab_business(t):
    ...
    if rv:
        ...
        st.warning(f"{icon} **Needs a quick human sign-off before acting**, {rv['reason']}. "
                   f"Routed to **{rv['impacted_owner']}**; suggested next step: {rv['recommended_action']}")
    ...
    if intent in ("refused", "clarify", "unsupported"):
        ...
        for cv in a.get("caveats", []):
            st.caption("• " + cv)
        if intent == "clarify":
            # Follow-through: let the user proceed with the governed default in one click.
            if st.button("Investigate digital conversion vs the prior 7-day average instead"):
                st.session_state["forced_question"] = P.DEMO_QUESTIONS[0]
                st.rerun()
        return
    ...
    if intent == "briefing":
        ...
        _biz_chart(t, "biz_briefing_chart")
        ...
        _biz_chart(t, "biz_overall_chart")
        _driver_block(a)
...
def _shared_trace_note(t):
    """Conversion-domain questions (the flagship + the per-analyst lenses) all run ONE shared
    cross-domain investigation, so these trace tabs are the same across them while only the
    Business answer changes, make that explicit so it doesn't read as a bug."""
    if (t.get("answer") or {}).get("intent") in ("overall", "driver"):
        st.caption("ℹ️ This is the **shared cross-domain investigation**, the full analyst team "
                   "runs on every conversion-domain question. Your question focused the **Business "
                   "answer**; the trace here is that same governed investigation.")
...
def _tab_team(t):
    ...
    if coord.get("retries"):
        st.caption(f"🔁 Targeted retries on failure: {coord['retries']} "
                   "(each failing analyst gets one repair attempt, then is escalated for review).")
...
def page_demo():
    ...
    st.caption("Governed Multi-Agent Analytics for Cross-Functional Retail Performance")
    st.write("Runs the real LangGraph pipeline on synthetic data. A team of specialized "
             "analysts is dispatched in parallel; results are exposed through seven views "
             "spanning the business answer, reasoning trace, and the multi-agent team.")
    probe = llm.probe()
    st.caption(f"LLM: {probe['detail']}")

    # A clarify answer can ask to proceed with the governed default; that sets a forced
    # question we auto-run on rerun.
    forced = st.session_state.pop("forced_question", None)
    ...
    options = (["·  Conversion-drop investigation  ·"] + P.DEMO_QUESTIONS
               ...
               + ["·  Edge-case scenarios  ·", P.TIE_SCENARIO_QUESTION, P.STALE_SCENARIO_QUESTION]
               + ["·  Direct analytics questions  ·"] + insights.questions()
               + ["·  Themed reviews (health / trend / risk)  ·"] + themes.questions()
               + ["✍️ Custom question…"])
    choice = st.selectbox("Pick an analytics question", options, index=1)
    if choice.startswith("·"):  # a group separator was selected
        choice = P.DEMO_QUESTIONS[0]
    # Edge-case scenarios run the full conversion investigation with a condition injected.
    scenario_tie = (choice == P.TIE_SCENARIO_QUESTION)
    scenario_stale = (choice == P.STALE_SCENARIO_QUESTION)
    if scenario_tie:
        st.caption("⚖️ Scenario: this investigates a seeded incident day on which the Electronics "
                   "stockout and the West fulfillment delays are genuinely near-equal in the data. "
                   "The team scores both from real DuckDB evidence; they tie on score and land "
                   "within the tie epsilon, so the deterministic 5-step tie-break can't separate "
                   "them, both are labeled possible contributors and routed to their owners for "
                   "review. No scores are injected; the tie emerges from the data.")
    if scenario_stale:
        st.caption("🕒 Scenario: the inventory feed is a day behind. The freshness gate checks the "
                   "governed source's latest load against the T-1 SLA, detects the lag from the "
                   "data itself, and routes the run to human review instead of answering on stale "
                   "data.")
    question = st.text_input("Question", value=forced or ("" if choice.startswith("✍️") else choice))
    ...
    if not (st.button("Run investigation", type="primary") or forced):
    ...
    if not question.strip():
        ...
        return
    # Edge-case scenarios run the full conversion investigation with a condition injected;
    # a clarify follow-through (forced) runs the governed default question.
    if forced:
        run_question, scenario_tie, scenario_stale = forced, False, False
    else:
        run_question = P.DEMO_QUESTIONS[0] if (scenario_tie or scenario_stale) else question
    with st.status("Running investigation…", expanded=False) as status:
        ...
        for kind, payload in run_investigation_stream(
                ...
                inject_stale=scenario_stale, top_k=top_k, beam_width=beam_width, depth=depth):
            if kind == "step":
...
def _arch_diagrams():
    ...
    for a in agent_specs.load_specs():
        with st.expander(f"🤖 {a.get('name')}: {a.get('description', '')}"):
            tools = a.get("tools")
            tools = ", ".join(tools) if isinstance(tools, list) else (tools or "n/a")
            st.markdown(f"**Owner:** {a.get('owner','n/a')}  ·  **Domain:** {a.get('domain','n/a')}  "
                        f"·  **Governed driver:** `{a.get('governed_driver','n/a')}`  "
                        f"·  **Metric:** `{a.get('metric','n/a')}`")
            st.markdown(f"**Tools:** {tools}")
            tbls = a.get("tables") or []
            st.markdown("**Tables:** " + (", ".join(f"`{t}`" for t in tbls) if tbls else "n/a"))
...
# ==========================================================================
def _arch_reference():
    # Reference diagrams, each in a collapsible expander and constrained to ~2/3 width so
    # they're not oversized; the fullscreen (⤢) icon zooms in for detail. Rendered only if
    # present so the page never breaks when an asset is absent.
    _archdir = _ROOT / "docs" / "architecture"
    diagrams = [
        ("📐 1 · Full project / reference architecture", "01_reference.png"),
        ("👥 2 · Supervisor-orchestrated multi-agent architecture", "02_multi_agent.png"),
        ("🔍 3 · Investigation workflow: the ReAct loop", "03_investigation_workflow.png"),
        ("🧠 4 · ReAct reasoning loop & semantic chunking", "04_react_chunking.png"),
        ("🚀 5 · Future-state (real-world enterprise) architecture", "05_future_state.png"),
    ]
    shown = [(title, fn) for title, fn in diagrams if (_archdir / fn).exists()]
    if not shown:
        st.info("Reference diagrams not found in `docs/architecture/`.")
        return
    st.caption("Open a diagram below; use the ⤢ (fullscreen) icon to zoom in for detail.")
    for i, (title, fn) in enumerate(shown):
        with st.expander(title, expanded=(i == 0)):
            _, mid, _ = st.columns([1, 4, 1])
            with mid:
                st.image(str(_archdir / fn), use_container_width=True)


def page_architecture():
    st.title("🏗️ Architecture")
    t_ref, t_sys, t_more = st.tabs(
        ["Reference diagrams", "System, knowledge graph & multi-agent design",
...
st.sidebar.title("🛍️ Omnichannel Retail Analytics Assistant")
st.sidebar.caption("Read-only · governed · multi-agent · synthetic data")
